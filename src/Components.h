...
struct Joint {
    std::string name = ""; //used by parser
    std::string id = ""; //used by parser
    GLint index_in_chain = -1; // what is the index of this joint given depth-first traversal of tree
    lm::mat4 matrix; // current model matrix, changed every frame in animation
    Joint* parent = nullptr; //pointer to parent joint
    std::vector<Joint*> children; //pointers to children
    std::vector<lm::mat4> keyframes; // each index is the frame with a new model matrix
    int num_keyframes = 0; //set by method below
    int current_keyframe = 0; //counter used by animation system
    
    //function receives a std::vector of floats which correspond to model matrices of all
    //keyframes of animation
    //obviously MUST be a factor or 16!
...
template<> struct type2int<JointChain> { enum { result = 8 }; };
//UPDATE THIS!
