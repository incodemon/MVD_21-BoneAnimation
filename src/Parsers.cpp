...
Joint* parseXMLJointNode(XMLElement* curr_node,
    ...
    for (child; child != NULL; child = child->NextSiblingElement("node")) {
        ...
        if (type == "JOINT") {
            ...
            new_joint->children.push_back(parseXMLJointNode(child, id_pointer_map, name_pointer_map, new_joint));
        }
    }
    return new_joint;
}
...
//opencollada parser
bool Parsers::parseColladaJoints(std::string filename,  GraphicsSystem& graphics_system) {
    ...
    for (child; child != NULL; child = child->NextSiblingElement("node")) {
        ...
        if ( type_attribute_string  == "JOINT") {
            ...
            Joint* root_joint = parseXMLJointNode(child, jointID_jointPointer, jointName_jointPointer);
            
            int joint_chain_ent = ECS.createEntity(name);
            JointChain& jc = ECS.createComponentForEntity<JointChain>(joint_chain_ent);
            jc.root = root_joint;
            jc.num_joints = (int)jointID_jointPointer.size();
        }
